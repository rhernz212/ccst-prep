 ...
 * tree. Where the vendor publishes no official per-domain scoring weight,
 * weight is a heuristic (equal share per objective count) — hence the
 * caller-supplied weightingMethod, which is carried into the output so the
 * choice stays discoverable rather than silently baked in.
 */
export function parseBlueprint(
  introXhtmlPath: string,
