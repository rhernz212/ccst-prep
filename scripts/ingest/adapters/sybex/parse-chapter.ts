...
export async function parseChapter(opts: ChapterContext): Promise<Chapter> {
  ...
  for (const navSection of opts.nav.sections) {
    ...
    // Not everything lives inside a <section>. Full-width figures are emitted
    // as siblings between one section's close and the next one's open, so
    // selecting section elements alone silently dropped them — two figures
    // went missing this way, each still referenced by a "FIGURE 8.8" link
    // that then pointed at nothing.
    //
    // Such content is appended to the section it follows, which is both its
    // position in reading order and, in both observed cases, the section
    // whose text references it.
    let trailingHtml = "";
    const trailing = sectionEl
      .nextUntil("section")
      .toArray()
      // <header> is the chapter number/title, rendered by the page itself.
      // Elements with neither text nor images are pagebreak markers — and
      // must be skipped rather than emitted, since a self-closed <span/>
      // would corrupt the document once re-parsed as HTML.
      .filter((el) => {
        if (el.tagName === "header") return false;
        const $el = $(el);
        return $el.text().trim().length > 0 || $el.find("img").length > 0;
      });

    for (const el of trailing) {
      const $el = $(el);
      stripEmptyBookmarkSpans($, $el);
      await rewriteImages($, $el, opts);
      trailingHtml += $.html(el);
    }
    ...
    sections.push({
      ...
      html: sanitizeChapterHtml((sectionEl.html() ?? "") + trailingHtml),
    });
  }
